...
const ApplicantSummary = () => {
  ...
  const navigateToLease = () => {
    axios
      .get(`http://64.225.8.160:4000/applicant/applicant_summary/${id}`)
      .then((response) => {
  ...
  useEffect(() => {
    axios
      ...
      .then((applicants) => {
        axios
          .get("http://64.225.8.160:4000/rentals/property")
          .then((properties) => {
  ...
  const fetchDataAndPost = async () => {
    try {
      ...
      const response = await axios.get(
        `http://64.225.8.160:4000/applicant/applicant_summary/${id}`
      );
